...
import { Common } from 'csharp';
import JSAdapter = Common.JSAdapter;

let Database = JSAdapter.sqlite3.Database
let Statement = JSAdapter.sqlite3.Statement
let Backup = JSAdapter.sqlite3.Backup
let sqlite3 = JSAdapter.sqlite3.sqlite3
...
JSAdapter.sqlite3.cached.prototype.Database = function(file: string, a: any, b: any) {
    if (file === '' || file === ':memory:') {
    ...
    if (!JSAdapter.sqlite3.cached.objects[file]) {
        db = JSAdapter.sqlite3.cached.objects[file] = new Database(file, a, b);
    } else {
        // Make sure the callback is called.
        db = JSAdapter.sqlite3.cached.objects[file];
...
sqlite3.verbose = function() {
    if (!isVerbose) {
        ...
        [ 'bind', 'get', 'run', 'all', 'each', 'map', 'reset',
            'finalize' ].forEach(function(name) {
...
export default {
    Database, Statement, Backup, sqlite3,
}

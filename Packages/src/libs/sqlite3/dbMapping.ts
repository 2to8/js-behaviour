...
import { Table, Column, PrimaryKey, AutoInc, Unique, NotNull, DefaultValue, MaxLength } from './utils/attribute';
...
class DBMapping {
    ...
    constructor(type: Type<object>) {
        ...
        if (this.columns.length <= 0) throw new Error(`数据表${ type.name }(${ this.tableName }), 没有有效字段`);
        
        let info = '';
        this.columns.forEach(col => info += '\n' + JSON.stringify(col));
        console.log(`DBMapping: ${ this.tableName }->Columns:${ info }`);
        //PK
        if (this.pk) this.getByPrimaryKeySql = `SELECT * FROM "${ this.tableName }" WHERE "${ this.pk.name }" = ?`; else this.getByPrimaryKeySql = `SELECT * FROM "${ this.tableName }" LIMIT 1`;
    }
    ...
    findColumnByPropertyName(name: string): DBColumn {
        for (let i = 0; i < this.columns.length; i++) {
            if (this.columns[i].prop == name) return this.columns[i];
        }
        return null;
    }
    
    dispose() {
        if (this._insertCommand) this._insertCommand.dispose();
    }

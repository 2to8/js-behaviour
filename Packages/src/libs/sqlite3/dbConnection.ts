import { GameEngine, SqlCipher4Unity3D, System, UnityEngine } from 'csharp';
import * as CS from 'csharp';
import { DBCommand } from './dbCommand';
import { DBMapping } from './dbMapping';
import { DBQuery } from './dbQuery';
import { Orm } from './orm';
import SQLiteConnection = SqlCipher4Unity3D.SQLiteConnection;
...
class DBConnection {
    ...
    instance<T1>(c: Type<T1>): T1
    {
        global.tables = global.tables || new Map()
        let id = c.name;
        Debug.Log(Strings.ToRed("table: "+id))
    
        if (!tables.has(id)) {
            let data = this.isUser ? global.db.table(c).firstOrDefault() : global.app.table(c).firstOrDefault();
            data.conn =  this.isUser ? global.db.table(c) : global.app.table(c);
            Debug.Log(Strings.ToRed("table: "+c.name+" data: " +JSON.stringify(data)))
            if(data == null){
                console.error(c.name + "is null")
            }
            if("init" in data){
                data.init();
            }
    ...
    set trace(val: boolean)
    {
        this._trace = val;
    }
    
    IsUser(val: boolean = true): this
    {
        this.isUser = val;
        return this;
    }
    ...
    {
        //if (!CS.System.IO.File.Exists(path)) throw new Error('FileNotFoundException: ' + path);
        this._opened = false;
        this._path = path;
        this._password = password;
    }
    
    open(force: boolean = false): this
    {
        if (!this._opened || !this._handle || force) {
            this._handle = new SQLiteConnection(this._path, this._password);
    ...
    {
        if (!this._opened) throw new Error('Cannot create commands from unopened database');
    ...
    {
        if (savepoint) this.doPoint(savepoint, 'ROLLBACK TO'); else this.executeUpdate('ROLLBACK');
    }
    ...
    {
        if (typeof (type) !== 'function') throw new Error('ctor is not constructor:' + type);
    ...
    private proofTable(mapping: DBMapping)
    {
        ...
        for (let i = 0; i < create_cols.length && !rebuild; i++) {
            ...
            if (i < exists_cols.length) {
                ...
                if (col1.name !== col2.name || col1.content !== col2.content) rebuild = true;
            } else add_cols.push(col1.content);
        }
        //重构表
        if (rebuild) {
            //寻找相同字段<继承>
            ...
            this.runInTransaction(() => {
                ...
                this.executeUpdate(`CREATE TABLE \"${ table_temp }\" AS
                SELECT *
                FROM \"${ table_name }\";`);
                this.executeUpdate(`DROP TABLE \"${ table_name }\";`);
                this.executeUpdate(create_sql);
                if (samecols.length > 0) this.executeUpdate(`INSERT INTO \"${ table_name }\" (${ samecols })
                                                             SELECT ${ samecols }
                                                             FROM \"${ table_temp }\";`);
        ...
        //表追加字段
            ...
            this.runInTransaction(() => {
                ...
                for (var col of add_cols) {
                    this.executeUpdate(`ALTER TABLE \"${ table_name }\"
                        ADD COLUMN ${ col };`);
    ...
    {
        if (!sql) throw new Error('Can\'t create a TableMapping instance, sql: ' + sql);
        let i1 = sql.indexOf('(');
        let i2 = sql.indexOf(')');
        sql = sql.substring(i1 + 1, i2).replace('\n', '').replace('\r', '');
        
        //console.log(sql);
        let fields: Array<{ name: string, content: string }> = [];
    ...
    static createFile(path: string, del_exists?: boolean): boolean
    {
        ...
        //     if (exists) CS.System.IO.File.Delete(path);
...
class DBConnectionBinding<T extends object> {
    ...
    get handle()
    {
        return this._conn.handle;
    }
    
    get datapath()
    {
        return this._conn.path;
    }
    
    get password()
    {
        return this._conn.password;
    }
    
    get opened()
    {
        return this._conn.opened;
    }
    
    get trace()
    {
        return this._conn.trace;
    }
    
    set trace(val: boolean)
    {
        this._conn.trace = val;
    }
...
export {
    DBConnection, DBConnectionBinding,
};
...
partial class Node {
    ...
    public Color customColor { get { return _color; } set { _color = value; } }

    ///EDITOR! Verbose level of the node GUI
    public VerboseLevel verboseLevel { get { return _verboseLevel; } set { _verboseLevel = value; } }
    ...
    private bool showIcon { get { return Prefs.showIcons && icon != null; } }
                ...
                if (assignable.subGraph == null) {
                    if (!Application.isPlaying && GUILayout.Button("CREATE NEW #0x02")) {
                        ...
                        var path = node.graph.agent.gameObject.GameObjectAssetPath()
                                ...
                                .Replace(".unity", "") +
                            "/Asset/" +
                            string.Join("-", node.graph.agent.name, assignable.subGraphParameter.varType.Name,
                                node.graph.agent.GetHashCode(), node.UID.Split('-').First()) +
                            ".asset";
                        path.CreateDirFromFilePath();

                        var newGraph = AssetDatabase.LoadAssetAtPath(path,assignable.subGraphParameter.varType) as Graph;

                        if (newGraph == null) {
                            newGraph = (Graph)EditorUtils.CreateAsset(assignable.subGraphParameter.varType, path);
                        }
        ...
        if (fullDrawPass || drawCanvas.Overlaps(rect)) {
            ...
            if (outConnections.Count < maxOutConnections || maxOutConnections == -1) {
                for (var i = 0; i < outConnections.Count + 1; i++) {
                    ...
                    portRect.center = new Vector2(((rect.width / (outConnections.Count + 1)) * (i + 0.5f)) + rect.xMin,
                        rect.yMax + 6);

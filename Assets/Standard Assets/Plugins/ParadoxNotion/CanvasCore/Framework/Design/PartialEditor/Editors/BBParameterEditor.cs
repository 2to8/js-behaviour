...
public static class BBParameterEditor {
        ...
        UnityEngine.Object context = null)
    {
        ...
        return ParameterField(string.IsNullOrEmpty(name) ? GUIContent.none : EditorUtils.GetTempContent(name), bbParam,
            blackboardOnly, false, info);
    }

    ///An editor for BBParameter type to let user choose either a constant value or link to a Blackboard Variable.
    public static BBParameter ParameterField(GUIContent content, BBParameter bbParam, bool blackboardOnly = false,
        bool required = false, InspectedFieldInfo info = default(InspectedFieldInfo))
    {
        if (bbParam == null) {
                ...
                if (bbParam.useBlackboard) {
                    ...
                    if (info.attributes != null) {
                        info.attributes = info.attributes
                            .Where(a => !(a is DrawerAttribute) || !((DrawerAttribute)a).isDecorator)
                            .ToArray();
                    }
                    GUILayout.BeginVertical();
    ...
    static void ParameterDropDown(GUIContent content, BBParameter bbParam)
    {
        if (bbParam.varRef == null && !string.IsNullOrEmpty(bbParam.name)) {
            ...
            if (bbParam.bb != null &&
                GUILayout.Button(EditorUtils.GetTempContent(Icons.plusIcon, "Promote To Variable"), Styles.centerLabel,
        ...
        var cachedContentTextForNew = string.IsNullOrEmpty(content.text)
            ? string.Format("new{0}", bbParam.varType.Name)
            : content.text;
        ...
        if (pop) {
            ...
            TagSystem.AllTags()
                .OrderBy(s => s)
                .ForEach(tag => {
                    menu.AddItem(new GUIContent("Tags/" + tag.Replace(".", "/")), false, () => {

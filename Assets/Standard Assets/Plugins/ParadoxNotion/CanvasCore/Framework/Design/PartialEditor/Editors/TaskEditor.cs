...
namespace NodeCanvas.Editor
{
    public class TaskEditor : EditorObjectWrapper<Task>
    {
        ...
        TaskSO m_TaskSo = ScriptableObject.CreateInstance<TaskSO>();
        ...
        {
            if (task != null) {
                ShowTaskInspectorGUI(task, callback, showTitlebar);
            }
        }
        ...
        void ShowInspector(Action<Task> callback, bool showTitlebar = true)
        {
            ...
            if (!showTitlebar || ShowTitlebar(callback) == true) {
                ...
                {
                    if (!m_Editor) {
                        m_TaskSo.m_节点数据 = task;
                        UnityEditor.Editor.CreateCachedEditor(m_TaskSo, null, ref m_Editor);
                    }

                    // Draw object properties
                    EditorGUI.BeginChangeCheck();
                    if (m_Editor) // catch empty property
                    {
                ...
                task.showDefaultUI = EditorGUILayout.Foldout(task.showDefaultUI, "数据面板",true);
                if (task.showDefaultUI) {
        ...
        void ShowAgentField()
        {
            ...
            if (task.agentIsOverride) {
                ...
                var compInfo = task.agent == null
                    ? task.agentType.FriendlyName().FormatError()

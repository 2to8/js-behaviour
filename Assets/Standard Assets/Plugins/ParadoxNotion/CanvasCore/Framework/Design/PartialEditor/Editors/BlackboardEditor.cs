...
namespace NodeCanvas.Editor
{
    ...
    public class BlackboardEditor : EditorObjectWrapper<IBlackboard>
    {
        ...
        {
            elementDefinedParameterIDs = element != null
                ? Graph.GetParametersInElement(element)
                    .Where(p => p != null && p.isDefined)
                    .Select(p => p.targetVariableID)
                : null;
        }
        ...
        void InspectorGUI(IBlackboard bb, UnityEngine.Object overrideContextObject = null)
        {
            ...
            if (!bb.unityContextObject.AssetPath().IsNullOrEmpty()) {
                if (bb.tempName.IsNullOrEmpty()) {
                    bb.tempName = Path.GetFileNameWithoutExtension(bb.unityContextObject.AssetPath());
                }
                GUILayout.BeginHorizontal();
                {
                    ...
                    bb.tempName = GUILayout.TextField(bb.tempName, GUILayout.ExpandWidth(true));
        ...
        void ShowDataLabelGUI(Variable data, int index)
        {
            ...
            var isVariablePrefabInstanceModified = variablesProperty != null
                ? variablesProperty.GetArrayElementAtIndex(index).prefabOverride
                : false;
        ...
        void ShowDataFieldGUI(Variable data, int index)
        {
            ...
            if (data.isPropertyBound) {
                ...
                GUILayout.Label(string.Format(".{0} ({1})", memberName, typeName.Split('.').Last()), Styles.leftLabel,
                    layoutOptions);
                GUI.color = Color.white;

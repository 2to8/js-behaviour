...
namespace NodeCanvas.Editor
{
    ...
    public partial class GraphEditor
    {
        ...
        public static Graph NewAsAsset(GraphOwner owner)
        {
            var path = owner.gameObject.GameObjectAssetPath().Replace(".prefab", "").Replace(".unity", "") +
                "/Asset/" +
                string.Join("-", owner.name, owner.graphType.Name, owner.GetHashCode()) +
                ".asset";
            path.CreateDirFromFilePath();

            var newGraph = AssetDatabase.LoadAssetAtPath<Graph>(path);

            if (newGraph == null) {
                newGraph = (Graph)EditorUtils.CreateAsset(owner.graphType, path);
            }

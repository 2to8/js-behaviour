...
public partial class SQLiteConnection : IDisposable {
    ...
    /// </param>
    public SQLiteConnection(string databasePath, bool storeDateTimeAsTicks) : this(
        new SQLiteConnectionString(databasePath, SQLiteOpenFlags.ReadWrite | SQLiteOpenFlags.Create,
    ...
    ///     the storeDateTimeAsTicks parameter.
    /// </param>
    public SQLiteConnection(string connectionString, string databasePath, SQLiteOpenFlags openFlags,
        bool storeDateTimeAsTicks = true) : this(new SQLiteConnectionString(databasePath, openFlags,
        storeDateTimeAsTicks)) { }
 ...
 SQLite3.Open (connectionString.DatabasePath, out handle, (int)connectionString.OpenFlags, connectionString.VfsName);
    ...
    public CreateTableResult CreateTable(Type ty, CreateFlags createFlags = CreateFlags.None)
    {
        ...
        if (map.Columns.Length == 0) {
            throw new Exception(
                string.Format("Cannot create a table without columns (does '{0}' have public properties?)",
                    ty.FullName));
        }
        ...
        if (existingCols.Count == 0) {
            ...
            var @using = fts3 ? "using fts3 " :
                fts4 ? "using fts4 " : string.Empty;
    ...
    /// Whether the table was created or migrated for each type.
    /// </returns>
    public CreateTablesResult CreateTables<T, T2, T3, T4>(CreateFlags createFlags = CreateFlags.None) where T : new()
    ...
    {
        if (!_open) throw SQLiteException.New(SQLite3.Result.Error, "Cannot create commands from unopened database");
    ...
    public bool CreateIfNotExists<T>() => CreateIfNotExists(typeof(T));

    public bool CreateIfNotExists(Type type)
    {
        var result = GetTableInfo(type.Name);
        if (!TableChecked.Contains(type) || result == null || result.Count == 0) {
            ...
            //     CreateTable(type, CreateFlags.AllImplicit);
            // }
            if (!TableChecked.Contains(type)) {
                TableChecked.Add(type);
            }
        }
        return true;
    }

    public static List<Type> TableChecked = new List<Type>();
#if UNITY_EDITOR
    [UnityEditor.Callbacks.DidReloadScripts]
    static void _ClearCache() => TableChecked.Clear();
#endif
    ...
    PreparedSqlLiteInsertCommand CreateInsertCommand(TableMapping map, string extra)
    {
        ...
        if (cols.Length == 0 && map.Columns.Length == 1 && map.Columns[0].IsAutoInc) {
            ...
            insertSql =
                $"insert {extra} into \"{map.TableName}\"({string.Join(",", (from c in cols select "\"" + c.Name + "\"").ToArray())}) values ({string.Join(",", (from c in cols select "?").ToArray())})";
        }

...
static class EditorHeaderGUID {
    ...
    static void CreateGraphOwner<T>(GameObject go) where T : GraphOwner
    {
    ...
    static void DisplayGUIDIfPersistent(Editor editor)
    {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            if (GUILayout.Button("B", GUILayout.Width(20))) {
                // MakeController(go);
                CreateGraphOwner<BehaviourTreeOwner>(go);
            }

            if (GUILayout.Button("F", GUILayout.Width(20))) {
                CreateGraphOwner<FSMOwner>(go);
            }

            if (GUILayout.Button("+", GUILayout.Width(20))) {
                go.GetComponents(typeof(Component))
                    .ForEach((c, n) => {

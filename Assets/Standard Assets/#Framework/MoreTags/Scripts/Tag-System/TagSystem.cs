...
namespace MoreTags
{
    public static class TagSystem
    {
                ...
                if (!m_References.Any()) {
                    ...
                    if (!Application.isPlaying) {
                        ...
                        Core.GetAllLoadedScenes()
                            ...
                            .ForEach(go => {
                                go.GetComponentsInChildren<Tags>(true)
                                    .Where(t => t != null && t.ids.Any())
                                    .ForEach(t => {
        ...
        public static T Find<T>(this GameObject[] parent, params string[] tags) where T : Component => query
            .Parent(parent)
            .tags(tags)
            .withTypes(typeof(T))
            .result.Select(go => go.GetComponent<T>())
            ...
            params (string[] tag, Type[] type)[] query)
        {
            ...
            query.ForEach((q, i) => {
                if (ret == null || ret.Any()) {
                    ret = q.tag.SelectMany(t => GetGameObjects(t)
                        .Where(go =>
                            go != null
                            && q.tag.All(nt => GetGameObjects(nt).Contains(go))
                            && (i == 0 || result[i - 1].Any(p => go.GetParents().Contains(p)))
                            && (q.type == null || !q.type.Any() || q.type.All(type => go.GetComponent(type) != null))));
                    result.Add(ret);
                }
            });
            ...
            return ret?.Where(t =>
                    parent == null
                    || parent.All(p => p?.gameObject == null)
                    || parent.Where(p => p?.gameObject != null)
                        .Any(p => t.transform.GetComponentsInParent<Transform>(true).Contains(p)))
                ?? new List<GameObject>();
        }
        ...
        public static void RenameTag(string old, string tag)
        {
            ...
            refs[tag] = refs[old];
            refs.Remove(old);
            TagData.FirstOrInsert(t => t.name == old, row => {
                row.name = tag;
            });
        ...
        public static string[] Tags(params int[] ids) => ids.Any()
            ? refs.Where(tk => ids.Contains(tk.Value.Id)).Select(tk => tk.Key).ToArray()
            : new string[] { };

        public static Dictionary<int, string> TagsId() => new Dictionary<int, string>().Of(tk => {
            refs.ForEach(k => tk[k.Value.Id] = k.Key);
        });
    ...
    #region HashSet Operation
        ...
        public static void SetText(GameObject go, object value)
        {
            ...
            has = has || go.GetComponent<RadialSlider>()?.Of(t => go.GetComponent<Image>().fillAmount = Convert.ToSingle(value)) != null;
